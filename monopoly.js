function Game() {
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		if (initiator.knowledgeChestJailCard) {
			currentTableRow = initiatorSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
		...
		} else if (recipient.knowledgeChestJailCard) {
			currentTableRow = recipientSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
		...
		if (initiator.bearbullJailCard) {
			...
			currentTableCell.textContent = "Get Out of Jail Free Card";
		} else if (recipient.bearbullJailCard) {
			currentTableRow = recipientSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
		...
		if (initiator.quizJailCard) {
			...
			currentTableCell.textContent = "Get Out of Jail Free Card";
		} else if (recipient.quizJailCard) {
			currentTableRow = recipientSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
	...
	var readTrade = function() {
		...
		var money;
		var knowledgeChestJailCard;
		var bearbullJailCard;
		var quizJailCard;
		...
		if (document.getElementById("tradeleftcheckbox42") && document.getElementById("tradeleftcheckbox42").checked) {
			quizJailCard = 1;
		} else if (document.getElementById("traderightcheckbox42") && document.getElementById("traderightcheckbox42").checked) {
			quizJailCard = -1;
		} else {
			quizJailCard = 0;
		}
		...
		var trade = new Trade(initiator, recipient, money, property, knowledgeChestJailCard, quizJailCard);
	...
	var writeTrade = function(tradeObj) {
		...
		if (document.getElementById("tradeleftcheckbox40")) {
			if (tradeObj.getknowledgeChestJailCard() === 1) {
		...
		if (document.getElementById("tradeleftcheckbox41")) {
			if (tradeObj.getbearbullJailCard() === 1) {
		...
		if (document.getElementById("tradeleftcheckbox42")) {
			if (tradeObj.getquizJailCard() === 1) {
				document.getElementById("tradeleftcheckbox42").checked = true;
			} else {
				document.getElementById("tradeleftcheckbox42").checked = false;
			}
		}

		if (document.getElementById("traderightcheckbox42")) {
			if (tradeObj.getquizJailCard() === -1) {
				document.getElementById("traderightcheckbox42").checked = true;
			} else {
				document.getElementById("traderightcheckbox42").checked = false;
			}
		}
	...
	this.acceptTrade = function(tradeObj) {
		...
		isAPropertySelected |= tradeObj.getknowledgeChestJailCard();
		isAPropertySelected |= tradeObj.getbearbullJailCard();
		isAPropertySelected |= tradeObj.getquizJailCard();
		...
		if (tradeObj.getknowledgeChestJailCard() === 1) {
			initiator.knowledgeChestJailCard = false;
			recipient.knowledgeChestJailCard = true;
			addAlert(recipient.name + ' received a "Get Out of Jail Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getknowledgeChestJailCard() === -1) {
			initiator.knowledgeChestJailCard = true;
			recipient.knowledgeChestJailCard = false;
			addAlert(initiator.name + ' received a "Get Out of Jail Free" card from ' + recipient.name + ".");
		}

		if (tradeObj.getbearbullJailCard() === 1) {
			initiator.bearbullJailCard = false;
			recipient.bearbullJailCard = true;
			addAlert(recipient.name + ' received a "Get Out of Jail Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getbearbullJailCard() === -1) {
			initiator.bearbullJailCard = true;
			recipient.bearbullJailCard = false;
			addAlert(initiator.name + ' received a "Get Out of Jail Free" card from ' + recipient.name + ".");
		}

		if (tradeObj.getquizJailCard() === 1) {
			initiator.quizJailCard = false;
			recipient.quizJailCard = true;
			addAlert(recipient.name + ' received a "Get Out of Jail Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getquizJailCard() === -1) {
			initiator.quizJailCard = true;
			recipient.quizJailCard = false;
			addAlert(initiator.name + ' received a "Get Out of Jail Free" card from ' + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		var reversedTrade = new Trade(recipient, initiator, -money, reversedTradeProperty, -tradeObj.getknowledgeChestJailCard(), -tradeObj.getbearbullJailCard(), -tradeObj.getquizJailCard());
	...
	this.bankruptcy = function() {
		...
		if (p.bearbullJailCard) {
			p.bearbullJailCard = false;
			pcredit.bearbullJailCard = true;
		}

		if (p.knowledgeChestJailCard) {
			p.knowledgeChestJailCard = false;
			pcredit.knowledgeChestJailCard = true;
		}

		if (p.quizJailCard) {
			p.quizJailCard = false;
			pcredit.quizJailCard = true;
		}
...
function Player(name, color) {
	...
	this.jail = false;
	this.jailroll = 0;
	this.knowledgeChestJailCard = false;
	this.bearbullJailCard = false;
	this.quizJailCard = false;
	this.bidding = true;
	this.human = true;

	this.stock = new Array(40).fill(0);	// stocks owned by player
	// this.AI = null;
...
// knowledgeChestJailCard: integer, 1 means offered, -1 means requested, 0 means neither
// bearbullJailCard: integer, 1 means offered, -1 means requested, 0 means neither
function Trade(initiator, recipient, money, property, knowledgeChestJailCard, bearbullJailCard, quizJailCard) {
	...
	this.getknowledgeChestJailCard = function() {
		return knowledgeChestJailCard;
	};

	this.getbearbullJailCard = function() {
		return bearbullJailCard;
	};

	this.getquizJailCard = function() {
		return quizJailCard;
	};
}
...
// PRINT INFORMATION ON THE CONTROL LOG
...
function popup(HTML, action, option) {
	...
	// BUY STOCK UNIT
	if (option === "stock") {
		document.getElementById("popuptext").innerHTML += "<div><input type=\"button\" value=\"No\" id=\"popupno\" /></div>";
		$("#popupclose").focus();
...
// DISPLAY UPDATED MONEY OF THE PLAYER TO THE MONEYBAR
...
// NOT USING: this game is not owning anything 
function updateOwned() {
	...
	if (p.quizJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox42' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Jail Free Card</td></tr>";
	}
...
// UPDATE PLAYER OWNED STOCKS
function updateStockOwned(p, position, units) {
	p.stock[position] += units;
	console.log(p.stock);
}
...
// CHANGE: Bear / Bull or Knowledge Chest
function quizBearbullKnowledgeChest() {
	var p = player[turn];

	// Bear/bull
	if (p.position === 2 || p.position === 7 || p.position === 17 || p.position === 22 || p.position === 33 || p.position === 36) {
		var bearbullIndex = bearbullCards.deck[bearbullCards.index];

		// Remove the get out of jail free card from the deck.
		if (bearbullIndex === 0) {
			bearbullCards.deck.splice(bearbullCards.index, 1);
		}

		popup("<img src='images/chance_icon.png' style='height: 50px; width: 26px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Bear/Bull:</div><div style='text-align: justify;'>" + bearbullCards[bearbullIndex].text + "</div>", function() {
			bearbullAction(bearbullIndex);
		});

		bearbullCards.index++;

		if (bearbullCards.index >= bearbullCards.deck.length) {
			bearbullCards.index = 0;
		}

	// knowledge chest	
	} else if (p.position === 4 || p.position === 12 || p.position === 28 || p.position === 38) {
		var knowledgeChestIndex = knowledgeChestCards.deck[knowledgeChestCards.index];

		// Remove the get out of jail free card from the deck.
		if (knowledgeChestIndex === 0) {
			knowledgeChestCards.deck.splice(knowledgeChestCards.index, 1);
		}

		popup("<img src='images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Knowledge Chest:</div><div style='text-align: justify;'>" + knowledgeChestCards[knowledgeChestIndex].text + "</div>", function() {
			knowledgeChestAction(knowledgeChestIndex);
		});

		knowledgeChestCards.index++;

		if (knowledgeChestCards.index >= knowledgeChestCards.deck.length) {
			knowledgeChestCards.index = 0;
		}

	// quiz
	} else if (p.position === 5 || p.position === 15 || p.position === 25 || p.position === 35) {
		var quizIndex = quizCards.deck[quizCards.index];

		// Remove the get out of jail free card from the deck.
		if (quizIndex === 0) {
			quizCards.deck.splice(quizCards.index, 1);
		}

		popup("<img src='images/chance_icon.png' style='height: 50px; width: 26px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Quiz</div><div style='text-align: justify;'>" + quizCards[quizIndex].text + "</div>", function() {
			quizAction(quizIndex);
		});

		quizCards.index++;

		if (quizCards.index >= quizCards.deck.length) {
			quizCards.index = 0;
		}
...
function bearbullAction(bearbullIndex) {
	var p = player[turn]; // This is needed for reference in action() method.

	// $('#popupbackground').hide();
	// $('#popupwrap').hide();
	bearbullCards[bearbullIndex].action(p);

	updateMoney();

	if (bearbullIndex !== 15 && !p.human) {
		p.AI.alertList = "";
		game.next();
	}
}

function quizAction(quizIndex) {
	var p = player[turn]; // This is needed for reference in action() method.

	// $('#popupbackground').hide();
	// $('#popupwrap').hide();
	quizCards[quizIndex].action(p);

	updateMoney();

	if (quizIndex !== 15 && !p.human) {
		p.AI.alertList = "";
		game.next();
	}
}

function knowledgeChestAction(knowledgeChestIndex) {
	var p = player[turn]; // This is needed for reference in action() method.

	// $('#popupbackground').hide();
	// $('#popupwrap').hide();
	knowledgeChestCards[knowledgeChestIndex].action(p);

	updateMoney();

	if (knowledgeChestIndex !== 15 && !p.human) {
		p.AI.alertList = "";
		game.next();
	}
}

// INCREASE MONEY OF CURRENT TURN PLAYER
...
// SUBTRACT MONEY OF CURRECT TURN PLAYER
...
function advanceToNearestRailroad() {
	...
	if (p.position < 15) {
		...
		p.position = 5;
		p.money += 20000;
		addAlert(p.name + " collected a $20000 salary for passing GO.");
	}
...
function payfifty() {
	...
	addAlert(p.name + " paid the $5000 fine to get out of jail.");
	updateMoney();
	updatePosition();
}
...
function useJailCard() {
	...
	if (p.knowledgeChestJailCard) {
		...
		knowledgeChestCards.deck.splice(knowledgeChestCards.index, 0, 0);
	...
	} else if (p.bearbullJailCard) {
		p.bearbullJailCard = false;

		// Insert the get out of jail free card back into the bearbull deck.
		bearbullCards.deck.splice(bearbullCards.index, 0, 0);
	...
	} else if (p.quizJailCard) {
		p.quizJailCard = false;

		// Insert the get out of jail free card back into the quiz deck.
		quizCards.deck.splice(quizCards.index, 0, 0);
...
// CAN BE USED TO MODIFY TO BUY STOCK FUNCTION
...
// CAN BE USED TO SELL STOCK FUNCTION
...
function sellStock() {
	...
	var sellStockIndex = parseInt(document.getElementById("stocksellIndex").value);
	var sellStockUnits = parseInt(document.getElementById("stocksellUnits").value);

	$("#statswrap").hide();
	$("#statsbackground").fadeOut(400);

	if (!Number.isInteger(sellStockIndex) || !Number.isInteger(sellStockUnits)) {
		popup("<p>ERROR: Please enter an integer.</p>");
	} else {
		if (sellStockIndex == 0 || sellStockIndex == 5 || sellStockIndex == 7 || sellStockIndex == 10 || sellStockIndex == 12 || sellStockIndex == 28 || sellStockIndex == 15 || sellStockIndex == 25 || 
			sellStockIndex == 35 || sellStockIndex == 17 || sellStockIndex == 22 || sellStockIndex == 33 || sellStockIndex == 36 || sellStockIndex == 20 || sellStockIndex == 30 || sellStockIndex == 2
			|| sellStockIndex == 4 || sellStockIndex == 38 || sellStockIndex < 0 || sellStockIndex >= 40 ||
			sellStockUnits > p.stock[sellStockIndex] || sellStockUnits <= 0) {
			popup("<p>ERROR: Please enter a correct Stock Index and Stock Units.</p>");
		} else {
			// sell stock by current stock price CHANGE
			var s = square[sellStockIndex];

			addamount(s.price*sellStockUnits, "selling " + sellStockUnits + " units of stock " + s.name);
			updateStockOwned(p, sellStockIndex, 0-sellStockUnits);
		}
	}
}
...
function showStocks() { 
	...
	var write;
	HTML
	HTML = "<div class='statsplayername'>" + p.name + "</div><table align='center'><tr>";
	HTML += "<td><div class='stockstatsTitle'>Index</div></td><td><div class='stockstatsTitle'>Stock Name</div></td><td><div class='stockstatsTitle'>Units</div></td><td><div class='stockstatsTitle'>Open Price</div></td><td><div class='stockstatsTitle'>Latest Price</div></td></tr>";
	for (var i = 0; i < 40; i++) {
		stock = square[i];
		if (i == 0 || i == 5 || i == 7 || i == 10 || i == 12 || i == 28 || i == 15 || i == 25 || 
			i == 35 || i == 17 || i == 22 || i == 33 || i == 36 || i == 20 || i == 30 || i == 2
			|| i == 4 || i == 38) {
				continue;
			} else {
				
				HTML += "<tr><td><div>" + i + "</div></td><td><div>" + stock.name + "</div></td><td><div>" + p.stock[i] + "</div></td><td><div>" + stock.currentprice +  "</div></td><td><div>" + stock.newprice1 + "</div></tr>" 
			}
	}

	HTML += "</table><div id='titledeed'></div>";

	HTML += "<div><label>Stock Index: </label><input id='stocksellIndex' style='width: 291px;' /></div>";
	HTML += "<div><label>Sell Units: </label><input id='stocksellUnits' style='width: 291px;' /></div>";
	HTML += "<div><input type='button' id='sellStockBtn' value='Sell' onclick='sellStock();'/>";
	document.getElementById("statstext").innerHTML = HTML;
	// Show using animation.
	$("#statsbackground").fadeIn(400, function() {
		$("#statswrap").show();
...
function showdeed(property) {
	...
	if (sq.mortgage) {
		...
		if (sq.groupNumber >= 3) {
			...
			document.getElementById("deed-currentprice").textContent = sq.currentprice;
			document.getElementById("deed-eps").textContent = sq.eps;
			document.getElementById("deed-dps").textContent = sq.dps;
			document.getElementById("deed-nta").textContent = sq.nta;
			document.getElementById("deed-pe").textContent = sq.pe;
			document.getElementById("deed-roe").textContent = sq.roe;
			document.getElementById("deed-newprice1").textContent = sq.newprice1;
...
// update buy stock
function stockbuy () {
	var buyStockPrice = parseInt(document.getElementById("stockbuy").value);
	$("#popupwrap").hide();
	$("#popupbackground").fadeOut(400);
	if (!Number.isInteger(buyStockPrice)) {
		popup("<p>ERROR: Please enter an integer.</p>");
	} else {
		payStock(buyStockPrice);
	}
}
...
function buy() {
	...
	// POP UP TO CHOOSE HOW MANY STOCK TO BUY
	popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Buy stock?</div><div>Per unit = $" + s.newprice1 + "<span id='stockprice'></span></div><div><input id='stockbuy' style='width: 291px;' /></div><div><input type='button' value='Buy' onclick='stockbuy();'/>", null, "stock");
}

// CHECK PLAYER ENOUGH MONEY TO BUY
function payStock(buyStockPrice) {
	var p = player[turn];
	var s = square[p.position];
	// var stock = square[p.position];
	var cost = s.newprice1 * buyStockPrice;
	...
	if (p.money >= cost) {
		...
		// property.owner = turn;
		updateMoney();
		addAlert(p.name + " bought " + buyStockPrice + " units of stock for " + cost + ".");
	
		// UPDATE STOCK BOUGHT TO PLAYER OBJECT
		updateStockOwned(p, p.position, buyStockPrice);
	
		$("#landed").hide();
	
	} else {
		popup("<p>" + p.name + ", you need $" + (buyStockPrice - p.money) + " more to buy this stock.</p>");
	}
...
// LAND ON CELL FUNCTION
function land(increasedRent) {
	increasedRent = !!increasedRent; // Cast increasedRent to a boolean value. It is used for the ADVANCE TO THE NEAREST RAILROAD/UTILITY bearbull cards.
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			// CHANGE THIS STOCK INFORMATION
			document.getElementById("landed").innerHTML = "<div>You landed on <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Buy stock($" + s.newprice1 + ")' title='Buy " + s.name + " for " + s.pricetext + ".'/></div>";
		}


		// game.addPropertyToAuctionQueue(p.position);
		// BUY STOCK FUNCTION HEREEEE
	}
	...
	// 		// CALL QUIZ FUNCTION
	// 		addAlert(p.name + " got a QUIZ.");

	// 	} else if (p.position == 4 || p.position == 12 || p.position == 28 || p.position == 38) { // Knowledge Chest
			
	// 		// CALL KNOWLEDGE CHEST
	// 		addAlert(p.name + " got a KNOWLEDGE CHEST.");
	...
	if (p.position == 5 || p.position == 15 || p.position == 25 || p.position == 35) { // Quiz 
		// CALL QUIZ FUNCTION
		addAlert(p.name + " got a QUIZ.");
	...
	if (!p.human) {
		popup(p.AI.alertList, quizBearbullKnowledgeChest);
		p.AI.alertList = "";
	} else {
		quizBearbullKnowledgeChest();
	}
}
...
function market(control_status) {
	// Use hardcode market data to deduct money from players

	if (control_status === 1){
		addAlert("Market is going uptrend!!");
		for (var i = 0; i < 40; i++) {
			sq = square[i];
			sq.newprice1=sq.newprice1*(1+((Math.floor(Math.random() * 15) + 1)/100));
			sq.newprice1 = Math.round((sq.newprice1 + Number.EPSILON) * 100) / 100;
			sq.pricechangeperc = Math.round(((sq.newprice1/sq.currentprice*100)+ Number.EPSILON) * 100) / 100;
		}

	} else if (control_status === -1){
		addAlert("Market is going downtrend!!");
		for (var i = 0; i < 40; i++) {
			sq = square[i];
			sq.newprice1=sq.newprice1*(1-((Math.floor(Math.random() * 15) + 1)/100));
			sq.newprice1 = Math.round((sq.newprice1 + Number.EPSILON) * 100) / 100;
		...
		market_status = Math.floor(Math.random() * 3);

		for (var i = 0; i < 40; i++) {
			sq = square[i];
			sq.newprice1=sq.currentprice;
			sq.pricechangeperc = Math.round(((sq.newprice1/sq.currentprice*100)+ Number.EPSILON) * 100) / 100;
		}
		...
		if (market_status === 2) {
			...
			addAlert("Market is going biasa!!");
			for (var i = 0; i < 40; i++) {
				sq = square[i];
				sq.newprice1=sq.currentprice;
				sq.pricechangeperc = Math.round(((sq.newprice1/sq.currentprice*100)+ Number.EPSILON) * 100) / 100;
...
function play() {
	...
	turn++;
	// Check if all player moved for each round
	if (turn > pcount) {
		turn -= pcount;
		...
		if (roundCnt >= MAX_ROUND) {
			// END GAME: see which player has the highest amount of money
		}
	}
	...
	if (p.jail) {
		...
		if (p.knowledgeChestJailCard || p.bearbullJailCard || p.quizJailCard) {
...
window.onload = function() {
	...
	knowledgeChestCards.deck = [];
	bearbullCards.deck = [];
	quizCards.deck = [];

	for (var i = 0; i < 16; i++) {
		bearbullCards.deck[i] = i;
		knowledgeChestCards.deck[i] = i;
		quizCards.deck[i] = i;
	}

	// Shuffle Chance and Community Chest decks.
	bearbullCards.deck.sort(function() {return Math.random() - 0.5;});
	knowledgeChestCards.deck.sort(function() {return Math.random() - 0.5;});
	quizCards.deck.sort(function() {return Math.random() - 0.5;});
	...
	for (var i = 0; i < 40; i++) {
		...
		//HTML += "<br /><div id='enlarge" + i + "pricechangeperc' class='enlarge-pricechangeperc'></div>";
	...
	for (var i = 0; i < 40; i++) {
		...
		//document.getElementById("enlarge" + i + "pricechangeperc").textContent = s.pricechangeperc;
	}
	...
	$("#sellStocksButton").on("click", showStocks);
	$("#sellStockBtn, #statsclose, #statsbackground").on("click", function() {
		$("#statswrap").hide();
		$("#statsbackground").fadeOut(400);
	});
